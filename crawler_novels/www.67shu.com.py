...
ROOT_URL = "https://www.67shu.com" # 网站根目录
GENERAL_PATH = ""                      # 通用路径
NOVEL_SUB_ID = "88/109794"              # 目录页面ID
ENCODING = "gbk"                    # 页面文字编码
CHAPTER_POST = 1
"https://www.67shu.com/88/109794/"
if GENERAL_PATH == "":
...
def rtn_chapter_list_info(html):
    ...
    novelName = soup.find_all(name="div", attrs={"class": "intro"})[0].h3.text
    novelName = novelName.split("《")[1]
    novelName = novelName.split("》")[0]
    # novelName = "妾本惊华"
    plog.debug("开始下载《{0}》".format(novelName))

    chapterListInfoSoup = soup.find_all(name="div", attrs={"class": "novel_list"})[0].find_all(name="dd")
...
def rtn_chapter_txt(chapterHtml):
    ...
    try:
        ...
        if "免费阅读！" in txtContent:
            txtContent = txtContent.split("免费阅读！")[1]
...
if __name__ == '__main__':
    ...
    for chapterInfo in chapterListInfo:
        ...
        chapterUrl = "{0}/{1}".format(FULL_URL, chapterInfo["href"])

        plog.debug("{3}/{4} 网址：{0}，页面章节标题：{2}，文件路径：{1} ！！！".format(chapterUrl, novelFilePath, chapterInfo["text"], n, len(chapterListInfo)))

        chapterHtml = get_html_all_content(chapterUrl, "yuedu_zhengwen", ENCODING)
