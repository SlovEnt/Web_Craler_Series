...
ROOT_URL = "https://www.kuaiyankanshu.net" # 网站根目录
GENERAL_PATH = ""                      # 通用路径
...
NOVEL_SUB_ID = "226269"              # 目录页面ID
ENCODING = "utf-8"                    # 页面文字编码
CHAPTER_POST = 1
"https://www.kuaiyankanshu.net/226269/dir.html"
if GENERAL_PATH == "":
    FULL_URL = "{0}/{1}/dir.html".format(ROOT_URL, NOVEL_SUB_ID)
else:
    FULL_URL = "{0}/{1}/{2}/dir.html".format(ROOT_URL, GENERAL_PATH, NOVEL_SUB_ID)
plog.debug("小说下载首页为：{0}".format(FULL_URL))


def rtn_chapter_list_info(html):
    soup = BeautifulSoup(html, 'html.parser')
    novelName = soup.find_all(name="div", attrs={"class": "header"})[0].h1.text
    ...
    chapterListInfoSoup = soup.find_all(name="ul", attrs={"class" : "dirlist"})[0].find_all(name="li")
...
def rtn_chapter_txt(chapterHtml):
    ...
    try:
        soupSub = soup.find_all(name="div", attrs={"class": "content"})[0]
...
if __name__ == '__main__':
    ...
    html = chrome_get_html_all_content(FULL_URL, "dirlist", ENCODING)
    ...
    for chapterInfo in chapterListInfo:
        ...
        chapterHtml = chrome_get_html_all_content(chapterUrl, "content", ENCODING)
